...
if (isset($_GET['id'])) {
    ...
    // Redirect to home page if no valid article ID is provided
    redirect('index.php');
}

// Article Query to fetch all data related to the particular article
...
if (isset($_SESSION['USER_ID'])) {
    // Bookmark Query to check if the particular article is bookmarked by the user
    ...
    if (mysqli_num_rows($bookmarkResult) > 0) {
        // Updating the variable to true to have bookmarked icon on the article card
        $bookmarked = true;
    }
}
...
if ($row > 0) {
    // Fetching the data of the particular record as an Associative Array
    while ($data = mysqli_fetch_assoc($result)) {
        ...
        $article_videos = $data['article_videos']; // Fetching video URLs
            ...
            <div class="container">
                <div class="page-container">
                    ...
                    <article class="card1">
                        ...
                        <!-- Article Image -->
                        <img src="./assets/images/articles/<?php echo htmlspecialchars($article_image); ?>" class="article-image" />
                        <!-- Container that stores author name, date and category name -->
                        <div class="meta">
                            <!-- Author Name -->
                            <div class="author">
                                <i class="fas fa-user-alt"></i><?php echo htmlspecialchars($author_name); ?>
                            </div>
                            ...
                            <!-- Category Name -->
                            <div class="tag <?php echo htmlspecialchars($category_color); ?>">
                                <?php echo htmlspecialchars($category_name); ?>
                            </div>
                        </div>
                        <!-- Article Description -->
                        <div class="article-content">
                            <p><?php echo nl2br(htmlspecialchars($article_desc)); ?></p>
                        </div>
                        <!-- Article Videos -->
                        <?php if (!empty($article_videos)): ?>
                            <div class="article-videos">
                                <h3>Related Videos</h3>
                                <?php
                                // Assuming videos is a comma-separated list of video URLs
                                $videoUrls = explode(',', $article_videos);
                                foreach ($videoUrls as $videoUrl): ?>
                                    <p><a href="<?php echo htmlspecialchars(trim($videoUrl)); ?>" target="_blank"><?php echo htmlspecialchars(trim($videoUrl)); ?></a></p>
                                <?php endforeach; ?>
                            </div>
                        <?php endif; ?>
                ...
                <aside>
                    ...
                    <div class="card2">
                        ...
                        if ($row > 0) {
                            ...
                            while ($data = mysqli_fetch_assoc($trendingResult)) {
                                // Storing the article data in variables
                                $trending_article_id = $data['article_id'];
                                $article_title = $data['article_title'];
                                $article_image = $data['article_image'];
                                $article_date = $data['article_date'];
                                $author_name = $data['author_name'];
                                ...
                                // Article date is updated to particular datetime format
                                ...
                                createAsideCard($article_image, $trending_article_id, $article_title, $author_name, $article_date);
                            }
                        }
                        ?>
                    ...
                    <div class="card2">
                        ...
                        <?php
                        // Related Article Query to fetch maximum of 5 random articles of the same category other than the present one
                        ...
                        if ($row > 0) {
                            ...
                            while ($data = mysqli_fetch_assoc($relatedResult)) {
                                // Storing the article data in variables
                                $related_article_id = $data['article_id'];
                                $article_title = $data['article_title'];
                                $article_image = $data['article_image'];
                                $article_date = $data['article_date'];
                                $author_name = $data['author_name'];
                                ...
                                // Calling user defined function to create an aside article card with respective article details
                                createAsideCard($article_image, $related_article_id, $article_title, $author_name, $article_date);
                            }
                        }
                        ?>
                        ...
                        <!-- View all button -->
                        <div class="text-center py-1">
                            <a href="articles.php?id=<?php echo $cat_id; ?>" class="btn btn-card">View All <span>&twoheadrightarrow; </span></a>
                        </div>
                    </div>
                </aside>
            </div>
